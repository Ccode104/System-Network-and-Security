...
void encrypt(char *plaintext,char*ciphertext,char key)
{
	...
	mpz_inits(plaintext_hash,key_hash,NULL);

	mpz_t keys[strlen(plaintext)];

	mpz_init(keys[0]);
	hash(key,key_hash);
	mpz_set(keys[0],key_hash);

	for(int i =1;i<strlen(plaintext);i++)
	{
		mpz_init(keys[i]);
		hash(plaintext[i-1],plaintext_hash);
		mpz_set(keys[i],plaintext_hash);
	}
...
void decrypt(char *decoded,char*ciphertext,char key)
{
	...
	mpz_inits(ciphertext_hash,key_hash,NULL);
	hash(key,key_hash);
	...
	for(int i = 0; i < strlen(ciphertext); i++)
	{
		...
		mpz_sub(diff,ciphertext_hash,key_hash);
...
void main(int argc,char *argv[])
{
	...
	decrypt(decoded,ciphertext,k_roll1);
